...
export class ArmyService {
  constructor(
    @InjectRepository(ArmyEntity) private readonly armyRepository: Repository<ArmyEntity>,
  ...
  public async createArmy(battleId: string, armyInfo: CreateArmyDTO): Promise<ArmyDTO> {
    const battle = await this.battleService.findOne(battleId);
    if(!battle) {
      throw new NotFoundException(`Battle with id: ${battle} doesn't exist!`); 
    }
    const army = this.armyRepository.create( {...armyInfo, battle: {id: battleId}}); 
    console.log(army);
